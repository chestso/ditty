#!/bin/bash
# build-ucrt64.sh - build ditty natively on Windows (MSYS2 UCRT64).
#
...
#
# Prerequisites (must be installed via pacman before running):
#   - mingw-w64-ucrt-x86_64-gc (Boehm GC)
#   - mingw-w64-ucrt-x86_64-pcre2 (PCRE2)
#   - boba (must be installed to $MINGW_PREFIX for TUI REPL; optional)
#
# Usage:
...
FIXSH_DIR="$(mktemp -d /tmp/ditty-fixsh.XXXXXX)"
