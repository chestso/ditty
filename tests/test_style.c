...
static void test_style_fallback_to_category(void)
{
    ...
    FlareStyleEntry kw_entry = { .fg_r = 255, .fg_g = 0, .fg_b = 0 };
    flare_style_set(s, HL_KEYWORD, &kw_entry);
    FlareStyleEntry result = flare_style_get(s, HL_KEYWORD_SPECIAL_FORM);
...
static void test_style_specific_overrides_category(void)
{
    FlareStyle *s = flare_style_new();
    FlareStyleEntry kw_entry = { .fg_r = 255, .fg_g = 0, .fg_b = 0 };
    FlareStyleEntry sf_entry = { .fg_r = 0, .fg_g = 255, .fg_b = 0 };
    flare_style_set(s, HL_KEYWORD, &kw_entry);
    flare_style_set(s, HL_KEYWORD_SPECIAL_FORM, &sf_entry);
    FlareStyleEntry result = flare_style_get(s, HL_KEYWORD_SPECIAL_FORM);
...
{
    FlareStyle *s = flare_style_new();
    FlareStyleEntry text_entry = { .fg_r = 200, .fg_g = 200, .fg_b = 200 };
    flare_style_set(s, HL_TEXT, &text_entry);
    FlareStyleEntry result = flare_style_get(s, HL_PUNCT_DOT);
...
{
    FlareStyle *s = flare_style_monokai();
    ASSERT_NOT_NULL(s);
    flare_style_free(s);
}
...
{
    FlareStyle *s = flare_style_monokai();
    FlareStyleEntry e = flare_style_get(s, HL_TEXT);
...
{
    FlareStyle *s = flare_style_monokai();
    FlareStyleEntry kw = flare_style_get(s, HL_KEYWORD);
    FlareStyleEntry txt = flare_style_get(s, HL_TEXT);
...
static void test_style_monokai_all_standard_types(void)
{
    ...
    for (int i = 0; i < (int)(sizeof(types) / sizeof(types[0])); i++) {
        FlareStyleEntry e = flare_style_get(s, types[i]);
        (void)e;
    }
    flare_style_free(s);
}
...
{
    FlareStyle *s = flare_style_monokai();
    FlareStyleEntry custom = { .fg_r = 0, .fg_g = 255, .fg_b = 0 };
    flare_style_set(s, HL_LITERAL_STRING, &custom);
    FlareStyleEntry result = flare_style_get(s, HL_LITERAL_STRING);
    ASSERT_EQ(result.fg_r, 0);
    ASSERT_EQ(result.fg_g, 255);
    flare_style_free(s);
}
