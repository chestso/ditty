...
static inline int utf8_is_emoji_presentation(uint32_t cp)
{
    if (cp == 0x231A || cp == 0x231B)
        return 1; /* ⌚⌛ watch, hourglass */
    if (cp >= 0x23E9 && cp <= 0x23F3)
        return 1; /* ⏩-⏳ media controls */
...
static inline int utf8_is_emoji_property(uint32_t cp)
{
    ...
    if (cp >= 0x2600 && cp <= 0x2605)
        return 1; /* ☀☁☂☃☄★ - but ★ is not emoji */
    if (cp == 0x2600 || cp == 0x2601 || cp == 0x2602 || cp == 0x2603 || cp == 0x2604)
        return 1;
    if (cp == 0x260E)
        return 1; /* ☎ telephone */
    if (cp == 0x2611)
    ...
    if (cp == 0x2622 || cp == 0x2623)
        return 1; /* ☢☣ radioactive, biohazard */
    if (cp == 0x2626)
        return 1; /* ☦ orthodox cross */
    if (cp == 0x262A)
        return 1; /* ☪ star and crescent */
    if (cp == 0x262E || cp == 0x262F)
        return 1; /* ☮☯ peace, yin yang */
    if (cp >= 0x2638 && cp <= 0x263A)
        return 1; /* ☸☹☺ */
    if (cp == 0x2640 || cp == 0x2642)
        return 1; /* ♀♂ gender symbols */
    if (cp >= 0x2648 && cp <= 0x2653)
        return 1; /* ♈-♓ zodiac */
    if (cp == 0x265F)
        return 1; /* ♟ chess pawn */
    ...
    if (cp == 0x2668)
        return 1; /* ♨ hot springs */
    if (cp == 0x267B)
        return 1; /* ♻ recycling */
    if (cp == 0x267E || cp == 0x267F)
        return 1; /* ♾♿ infinity, wheelchair */
    if (cp >= 0x2692 && cp <= 0x2697)
        return 1; /* ⚒⚓⚔⚕⚖⚗ tools */
    if (cp == 0x2699)
        return 1; /* ⚙ gear */
    if (cp == 0x269B || cp == 0x269C)
        return 1; /* ⚛⚜ atom, fleur-de-lis */
    if (cp == 0x26A0 || cp == 0x26A1)
        return 1; /* ⚠⚡ warning, lightning */
    if (cp == 0x26A7)
        return 1; /* ⚧ transgender */
    if (cp == 0x26AA || cp == 0x26AB)
        return 1; /* ⚪⚫ circles */
    if (cp == 0x26B0 || cp == 0x26B1)
        return 1; /* ⚰⚱ coffin, urn */
    if (cp == 0x26BD || cp == 0x26BE)
        return 1; /* ⚽⚾ soccer, baseball */
    if (cp == 0x26C4 || cp == 0x26C5)
        return 1; /* ⛄⛅ snowman, sun behind cloud */
    if (cp == 0x26C8)
        return 1; /* ⛈ thunder cloud */
    if (cp == 0x26CE || cp == 0x26CF)
        return 1; /* ⛎⛏ ophiuchus, pick */
    if (cp == 0x26D1)
        return 1; /* ⛑ helmet */
    if (cp == 0x26D3 || cp == 0x26D4)
        return 1; /* ⛓⛔ chains, no entry */
    if (cp == 0x26E9 || cp == 0x26EA)
        return 1; /* ⛩⛪ shrine, church */
    ...
    if (cp == 0x274C)
        return 1; /* ❌ cross mark */
    if (cp == 0x274E)
        return 1; /* ❎ cross mark button */
    if (cp >= 0x2753 && cp <= 0x2755)
        return 1; /* ❓❔❕ */
    if (cp == 0x2757)
        return 1; /* ❗ exclamation */
    if (cp == 0x2763 || cp == 0x2764)
        return 1; /* ❣❤ hearts */
    if (cp >= 0x2795 && cp <= 0x2797)
        return 1; /* ➕➖➗ math */
    ...
    if (cp == 0x27B0)
        return 1; /* ➰ curly loop */
    if (cp == 0x27BF)
        return 1; /* ➿ double curly loop */
    /* Other blocks with Emoji=Yes */
    if (cp == 0x203C)
        return 1; /* ‼ double exclamation */
    if (cp == 0x2049)
        return 1; /* ⁉ exclamation question */
    ...
    if (cp == 0x3030)
        return 1; /* 〰 wavy dash */
    if (cp == 0x303D)
        return 1; /* 〽 part alternation mark */
    if (cp == 0x3297)
